...
def strategy2(b=s3.body):
    ...
    if canvas.frame%10==0: # кожні n кадірів
        ...
        if b.action: # якщо змінювати
            target_angle = 2 * math.pi * random.random()#зробив плавнішу зміну кута
            angle_diff = target_angle - b.angle
            b.angle += angle_diff * 0.05
            b.angle = b.angle % (2 * math.pi)

        if R>180: # запобігти виїзду за межі
            b.angle=getAngle(x,y,350,250)

        #Добавив зміну напрямку, якщо робот занадто близько до s0 або s2
        distance_to_s0 = getDist(x, y, s0.body.position[0], s0.body.position[1])
        distance_to_s2 = getDist(x, y, S2[0].body.position[0], S2[0].body.position[1])

        if distance_to_s0 < 40 or distance_to_s2 < 20:
            random_angle = 2 * math.pi * random.random()
            b.angle = random_angle

        #Добавив хапам'ятовування роботом попередніх 30 позицій, щоб він не повертався в ті області, якщо об'єкта у них немає
        ...
        for px, py in previous_positions:
            if not inCircle(px, py, 350, 250, 20):
                avoid_previous = False

        if avoid_previous:
            b.angle += math.pi / 2

        distance_to_s1 = getDist(x, y, s1.body.position[0], s1.body.position[1])
        if distance_to_s1 < 60:  # Добавив новий стан - відстань до об'єкта
            b.angle = getAngle(x, y, s1.body.position[0], s1.body.position[1])
